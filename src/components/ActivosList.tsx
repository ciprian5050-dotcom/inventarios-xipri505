...
export function ActivosList({ activos, cuentadantes, onEdit, onDelete }: ActivosListProps) {
  ...
  // Asegurar que activos sea un array antes de hacer .map()
  const activosArray = Array.isArray(activos) ? activos : [];
  
  const dependencias = Array.from(new Set(activosArray.map(a => a.dependencia)));
  const estados = ['Todos', 'Activo', 'Inactivo', 'En mantenimiento', 'Dado de baja', 'Extraviado'];

  const filteredActivos = activosArray.filter(activo => {
    const matchesSearch = 
