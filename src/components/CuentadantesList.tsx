...
export function CuentadantesList({ cuentadantes, onEdit, onDelete }: CuentadantesListProps) {
  ...
  // Validación defensiva: asegurar que cuentadantes sea un array
  const safeCuentadantes = Array.isArray(cuentadantes) ? cuentadantes : [];

  const filteredCuentadantes = safeCuentadantes.filter(cuentadante => {
    const dependenciasStr = cuentadante.dependencias && cuentadante.dependencias.length > 0
      ? cuentadante.dependencias.join(' ')
      : cuentadante.dependencia || '';
    ...
    const matchesSearch = 
      ...
      dependenciasStr.toLowerCase().includes(searchTerm.toLowerCase());

    return matchesSearch;
  });

  // 🔍 VALIDACIÓN: Asegurar que todos los cuentadantes tengan ID único y válido
  console.log('🔍 DEBUG Cuentadantes:', {
    total: filteredCuentadantes.length,
    ids: filteredCuentadantes.map(c => c.id),
    duplicados: filteredCuentadantes.filter((c, i, arr) => arr.findIndex(a => a.id === c.id) !== i).length,
    sinId: filteredCuentadantes.filter(c => !c.id).length
  });

  // ✅ FILTRAR elementos sin ID o con IDs duplicados para evitar warning de React
  const validCuentadantes = filteredCuentadantes.filter((c, index, arr) => {
    // Debe tener ID
    if (!c.id) return false;
    // No debe ser duplicado (solo mantener la primera ocurrencia)
    return arr.findIndex(item => item.id === c.id) === index;
  });
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        ...
        <table className="w-full">
          ...
          <tbody>
            {validCuentadantes.length === 0 ? (
              ...
              validCuentadantes.map((cuentadante) => {
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-900">
                          {cuentadante.dependencias && cuentadante.dependencias.length > 0 
                            ? cuentadante.dependencias.join(', ') 
                            : cuentadante.dependencia}
                        </span>
                      </div>
      ...
      <div className="px-6 py-4 bg-slate-50 border-t border-slate-200">
        <p className="text-slate-600">
          Mostrando {validCuentadantes.length} de {safeCuentadantes.length} cuentadantes
        </p>
      </div>
    </div>
  );
